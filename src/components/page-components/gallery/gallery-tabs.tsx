...
>(({ className, children, ...props }, ref) => (
	<SelectPrimitive.Item
		ref={ref}
		className={cn(
			'relative flex w-full cursor-default select-none items-center rounded-sm py-[10px] pl-2 text-center text-[18px] font-bold leading-[21px] text-black outline-none first:pt-0 data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
			className
		)}
		{...props}
	>
...
>(({ className, children, ...props }, ref) => (
	...
	>
		...
		<SelectPrimitive.Icon asChild>
			<img className="ml-2 h-[3.7px] w-[13.5px] object-cover" src={ChevronDownIcon.src} />
...
>(({ className, children, position = 'popper', ...props }, ref) => (
	<SelectPrimitive.Portal>
		<SelectPrimitive.Content
			ref={ref}
			className={cn(
				'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-b-md border border-t-0 border-[#999999] bg-white shadow-md',
				position === 'popper' &&
